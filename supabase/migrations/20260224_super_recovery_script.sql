-- Migration: SUPER RECOVERY SCRIPT (Consolidated Infrastructure)
...
CREATE TABLE IF NOT EXISTS public.financial_movements (
    ...
    reversal_of UUID REFERENCES public.financial_movements(id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    amount NUMERIC GENERATED ALWAYS AS (debit - credit) STORED
);
...
ALTER TABLE public.financial_movements 
    DROP CONSTRAINT IF EXISTS financial_movements_debit_credit_non_negative;
ALTER TABLE public.financial_movements
    ADD CONSTRAINT financial_movements_debit_credit_non_negative
        CHECK (debit >= 0 AND credit >= 0);

ALTER TABLE public.financial_movements 
    DROP CONSTRAINT IF EXISTS financial_movements_single_side;
ALTER TABLE public.financial_movements
