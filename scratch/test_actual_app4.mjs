...
const ocr = fs.readFileSync('scratch/ocr_latest.txt', 'utf8');
...
const dedupePreviewRows = (rows) => {
    ...
    const scoreRow = (row) => {
        ...
        else if (source === 'anchor') s += 30;
        else if (source === 'datetalao') s += 10;
        else if (source === 'compact') s += 0;

        return s;
    };

    for (const r of rows) {
        const key = String(r._talao || '').trim();
        if (!key) continue;
        ...
        if (!grouped.has(key)) {
            ...
            if (scoreRow(r) > scoreRow(existing)) {
                grouped.set(key, r);
            }
        }
    }

    return Array.from(grouped.values()).sort((a, b) => (a._manualDate || '').localeCompare(b._manualDate || ''));
};
...
const deduped = dedupePreviewRows(all);
console.log(`Encontrados: ${deduped.length}`);
const sum = deduped.reduce((acc, t) => acc + (t.Total || 0), 0);
console.log(`Soma: ${sum.toFixed(2)}`);
deduped.forEach(t => console.log(`${t['Matrícula']} | ${t._manualDate} | ${t._talao} | ${t.Total}`));
