...
const normalizeFuelToken = (val) => {
    ...
    if (raw.includes('ULTIMATE') || raw.includes('ULT'))
        return 'Gasóleo Ultimate';
    if (raw.includes('GASOLINA'))
        return 'Gasolina';
    if (raw.includes('ADBLUE'))
        return 'AdBlue';
    if (raw.includes('GPL') || raw.includes('GNV'))
        return 'GPL';
    return 'Gasóleo'; // Default fallback
};
const fixPlateOCR = (plate) => {
...
async function extractLinesFromPDF(fileData) {
...
function segmentIntoCardBlocks(lines) {
    const blocks = [];
...
function extractTransactionsFromBlock(block, invoiceRef) {
    const transactions = [];
    ...
    for (const lineTokens of block.lines) {
        ...
        if (liters <= 0)
            continue; // Failed strict validation
        transactions.push({
...
// ---- MAIN ENTRY POINT ----
export async function parseBPInvoicePDF(file) {
    let fileData;
