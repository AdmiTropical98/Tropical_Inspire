...
import DriverAvailabilityTab from './DriverAvailability';
...
export default function Escalas() {
    ...
    const [activeTab, setActiveTab] = useState<'gestao' | 'disponibilidade'>('gestao');
    ...
    const handleRunAutomation = async () => {
        ...
        try {
            ...
            const suggested = suggestDrivers(
                groups,
                motoristas,
                servicos.filter(s => s.data === selectedDate),
                autoSettings.activeCenter
            );
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
                        ...
                        <div className="flex bg-[#1e293b] rounded-lg p-1 border border-white/5 shrink-0">
                            <button
                                ...
                                className={`p-1.5 px-3 rounded-md transition-all text-xs font-bold uppercase tracking-wider ${activeTab === 'gestao' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}
                            >
                                Gestão
                            </button>
                            <button
                                onClick={() => setActiveTab('disponibilidade')}
                                className={`p-1.5 px-3 rounded-md transition-all text-xs font-bold uppercase tracking-wider ${activeTab === 'disponibilidade' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}
                            >
                                Disponibilidade
                            </button>
                        </div>
