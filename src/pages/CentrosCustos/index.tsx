...
function CostTooltip({ items }: { items: { label: string; value: number; color: string }[] }) {
    ...
    return (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 bg-white/90 border border-slate-200 rounded-xl p-3 shadow-2xl min-w-[200px] text-xs pointer-events-none">
            ...
            {items.map(i => (
                <div key={i.label} className="flex justify-between items-center gap-4 py-0.5">
                    ...
                    <div className="text-right">
                        <span className="text-slate-900 font-bold">{eur(i.value)}</span>
            ...
            <div className="border-t border-slate-200 mt-2 pt-2 flex justify-between">
                ...
                <span className="text-slate-900 font-black">{eur(total)}</span>
            </div>
        </div>
    );
}
...
function CostBar({ fuel, charging, tolls, reqs, labor }: { fuel: number; charging: number; tolls: number; reqs: number; labor: number }) {
    ...
    return (
        <div className="relative" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
            ...
            <div className="h-2.5 w-full bg-white/90 rounded-full overflow-hidden flex cursor-help shadow-inner">
                {items.map(i => (
...
function HealthBadge({ variation }: { variation: number | null }) {
    if (variation === null) return <span className="text-xs text-slate-500 font-bold px-2 py-0.5 rounded-full bg-slate-100">Novo</span>;
        ...
        <div className="animate-in fade-in duration-500">
            ...
            <div className="p-4 md:p-8 space-y-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                    <div className="col-span-2 md:col-span-3 xl:col-span-1 bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-200 p-5 rounded-3xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-20 h-20 bg-blue-500/10 rounded-full blur-2xl" />
                        <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Despesa Global</p>
                        <p className="text-2xl font-black text-slate-900">{eur(grandTotal)}</p>
                        <div className="mt-3 h-1.5 w-full bg-white rounded-full overflow-hidden flex">
                    ...
                    {[
                        ...
                        <div key={k.label} className={`${k.color} border p-4 rounded-3xl backdrop-blur-md`}>
                            ...
                            <p className="text-lg font-bold text-[#1f2957]">{eur(k.val)}</p>
                                ...
                                {r.items.map((item, i) => (
                                    <div key={item.label} className="flex items-center gap-3 py-1.5 border-b border-white/5 last:border-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-slate-900 text-xs font-bold truncate">{item.label}</p>
                                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                    {enriched.length === 0 ? (
                        <div className="col-span-full flex flex-col items-center justify-center p-20 bg-white/90/20 rounded-3xl border-2 border-dashed border-slate-200">
                        ...
                        >
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex items-start justify-between mb-5">
                                    <div className="flex items-center gap-3">
                                        <div className="w-12 h-12 bg-blue-500/10 rounded-2xl flex items-center justify-center border border-blue-500/20 group-hover:bg-blue-600 group-hover:border-blue-500 transition-all">
                                            <Building2 className="w-6 h-6 text-blue-400 group-hover:text-slate-900 transition-colors" />
                                        </div>
                                        <div>
                                            <h3 className="text-lg font-black text-slate-900 group-hover:text-blue-400 transition-colors uppercase tracking-tight">{cc.nome}</h3>
                                            {cc.localizacao && (
                                ...
                                <div className="mb-4">
                                    ...
                                    <p className="text-3xl font-black text-slate-900 tracking-tighter">{eur(cur.total)}</p>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-5">
                                    <div className="bg-white/90 border border-slate-200/50 p-2.5 rounded-xl text-center">
                                        <p className="text-[9px] text-slate-500 font-black uppercase mb-1">Motoristas</p>
                                        <p className="text-sm font-bold text-slate-900">{cur.drivers.length}</p>
                                    </div>
                                    <div className="bg-white/90 border border-slate-200/50 p-2.5 rounded-xl text-center">
                                        <p className="text-[9px] text-slate-500 font-black uppercase mb-1">Litros</p>
                                        <p className="text-sm font-bold text-slate-900">{cur.fuelLiters.toFixed(0)}L</p>
                                    </div>
                                    <div className="bg-white/90 border border-slate-200/50 p-2.5 rounded-xl text-center">
                                        <p className="text-[9px] text-slate-500 font-black uppercase mb-1">€/Litro</p>
                                        <p className="text-sm font-bold text-slate-900">{cur.eurPerLitre > 0 ? cur.eurPerLitre.toFixed(3) : '—'}</p>
                                    </div>
                                </div>
                ...
                {selectedCC && selectedEnriched && (
                    <div className="fixed inset-0 bg-black/90 backdrop-blur-2xl flex items-center justify-center z-[100] p-4">
                        <div className="bg-[#0f172a] w-full max-w-5xl h-[90vh] rounded-[2rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col">
                            ...
                            {/* Modal header */}
                            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/90 flex-shrink-0">
                                <div className="flex items-center gap-3">
                                    <button onClick={() => setSelectedCC(null)} className="p-2 hover:bg-white/5 rounded-xl transition-colors text-slate-400 hover:text-slate-900">
                                        <ChevronLeft className="w-5 h-5" />
                                    </button>
                                    <div>
                                        <h2 className="text-2xl font-black text-slate-900 tracking-tighter">{selectedCC.nome}</h2>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button onClick={() => setSelectedCC(null)} className="p-2 bg-slate-100 text-slate-400 hover:text-slate-900 rounded-xl">
                                        <X className="w-5 h-5" />
                                    </button>
                                </div>
                            </div>
