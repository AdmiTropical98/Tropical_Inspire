...
export default function ClientProfile() {
    ...
    const load = async () => {
      ...
      try {
        const [clientResult, historyResult] = await Promise.all([
          supabase
            .from('clientes')
            .select('id, nome, nif, email, telefone')
            .eq('id', clientId)
            .single(),
          supabase
            .from('requisicoes')
            .select('id, numero, data, custo, approved_value, status, fornecedor:fornecedores(nome), viatura:viaturas(matricula, marca, modelo)')
        ...
        setClient(clientResult.data as ClientInfo);
        setHistory((historyResult.data || []) as ClientHistoryRow[]);
      } catch (error) {
  ...
  const totals = useMemo(() => {
    const totalSpent = history.reduce((acc, row) => acc + rowAmount(row), 0);
    ...
    return {
      totalSpent,
      requisitionsCount: history.length,
  ...
  if (loading) {
    return <div className="p-8 text-slate-400">A carregar perfil do cliente...</div>;
  }
  ...
  if (!client) {
    return (
      <div className="space-y-4 p-8 text-slate-300">
        ...
        <Link to="/clientes" className="text-blue-400 hover:text-blue-300">Voltar para clientes</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <SummaryCard title="Total de despesas" value={formatCurrency(totals.totalSpent)} icon={<Euro className="h-4 w-4" />} />
        <SummaryCard title="Número de requisições" value={String(totals.requisitionsCount)} icon={<ClipboardList className="h-4 w-4" />} />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white/90 p-5">
        ...
        <div className="overflow-x-auto">
          ...
          {history.length === 0 && (
            <p className="py-3 text-sm text-slate-500">Este cliente ainda não tem histórico de despesas.</p>
          )}
        </div>
      </section>
