...
export default function UserPermissionsModal({ isOpen, onClose, user, onSave }: UserPermissionsModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
            <div className="bg-[#1e293b] w-full max-w-4xl rounded-3xl border border-slate-200 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-slate-200 bg-slate-100 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">
                            Utilizador: <span className="text-slate-900 font-medium">{user.nome}</span> | Perfil: <span className="text-blue-400">{user.role}</span>
                        </p>
                    </div>
                    <button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-700 rounded-full transition-all">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                ...
                {/* Footer */}
                <div className="p-6 border-t border-slate-200 bg-slate-100 flex justify-between items-center">
                    ...
                    <div className="flex gap-3">
                        <button
                            onClick={onClose}
                            className="px-6 py-2.5 text-slate-400 hover:text-slate-900 hover:bg-slate-700 rounded-xl transition-all"
                        >
                            Cancelar
                        </button>
                        <button
