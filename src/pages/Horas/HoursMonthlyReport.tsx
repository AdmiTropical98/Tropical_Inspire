...
export default function HoursMonthlyReport({ selectedMonth }: HoursMonthlyReportProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/5 flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-2xl font-black text-white">{globalStats.totalHours}</h3>
                    </div>
            ...
            {/* Header / Actions */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900/80 p-4 rounded-3xl border border-white/5 backdrop-blur-md">
                <div className="flex items-center gap-4">
                    <div className="p-2.5 bg-blue-600/10 rounded-xl">
                        <FileSpreadsheet className="w-5 h-5 text-blue-500" />
                    </div>
                    <div>
                        <h2 className="text-white font-black uppercase tracking-tighter text-lg">Relatório Payroll</h2>
                        <span className="text-slate-500 text-xs font-mono">{selectedMonth}</span>
                    </div>
                </div>

                <div className="flex gap-2 w-full md:w-auto">
                    <button onClick={generateExcel} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all border border-white/5">
            ...
            {/* Main Table */}
            <div className="bg-slate-900/50 border border-white/5 rounded-3xl overflow-hidden shadow-2xl overflow-x-auto table-scroll">
                        ...
                        {data.length === 0 ? (
                            ...
                            data.map((row, i) => (
                                <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                                    <td className="p-5 font-black text-white uppercase tracking-tighter">{row.driverName}</td>
                                    <td className="p-5 text-center">
                                        <span className="bg-slate-800 px-2 py-1 rounded-lg text-slate-400 font-mono">{row.daysWorked}</span>
                                    </td>
