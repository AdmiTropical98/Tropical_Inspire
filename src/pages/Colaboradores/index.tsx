...
import { CheckCircle2, Edit3, IdCard, Plus, QrCode, RefreshCcw, ScanLine, Search, Trash2, UserRound, Users, XCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pb-24">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-100/90 border border-slate-200 rounded-2xl p-4 shadow-sm">
          <p className="text-xs text-slate-500 uppercase font-bold tracking-widest">Colaboradores Ativos</p>
          <p className="text-3xl font-black text-slate-900 mt-2">{totalAtivos}</p>
        </div>
        <div className="bg-slate-100/90 border border-slate-200 rounded-2xl p-4 shadow-sm">
          <p className="text-xs text-slate-500 uppercase font-bold tracking-widest">Total Registados</p>
          <p className="text-3xl font-black text-slate-900 mt-2">{colaboradores.length}</p>
        </div>
        <div className="bg-slate-100/90 border border-slate-200 rounded-2xl p-4 shadow-sm">
          <p className="text-xs text-slate-500 uppercase font-bold tracking-widest">Inativos</p>
          <p className="text-3xl font-black text-slate-900 mt-2">{Math.max(colaboradores.length - totalAtivos, 0)}</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-slate-100/90 p-1.5 shadow-sm">
        <button
          type="button"
          onClick={() => {
            if (isScannerOpen) {
          ...
          className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition-colors ${
            activeView === 'colaboradores' ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-white'
          }`}
        >
        ...
        <button
          type="button"
          onClick={() => setActiveView('leitor')}
          className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition-colors ${
            activeView === 'leitor' ? 'bg-emerald-600 text-white' : 'text-slate-700 hover:bg-white'
          }`}
        >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[360px_1fr] gap-6">
          <div className="bg-white/90 border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            <form onSubmit={handleSalvar} className="space-y-4">
              <div>
                <label className="block text-xs text-slate-500 font-bold uppercase tracking-wider mb-1">Número de colaborador</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs text-slate-500 font-bold uppercase tracking-wider mb-1">Nome</label>
                <input
                  type="text"
                  value={formData.nome}
              ...
              <div>
                <label className="block text-xs text-slate-500 font-bold uppercase tracking-wider mb-1">Paragem</label>
                <input
                  type="text"
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="button"
                  onClick={resetForm}
                  className="px-4 rounded-xl bg-slate-100 text-slate-700 hover:text-slate-900 border border-slate-300"
                >
                  Limpar
                </button>
              </div>
          ...
          <div className="bg-white/90 border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between mb-4">
              <h2 className="text-slate-900 font-black text-lg tracking-tight">Colaboradores de Transporte</h2>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="bg-slate-50 border border-slate-300 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-900"
                  />
                </div>
                ...
                <select
                  ...
                  className="bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900"
                >
                ...
                <button
                  onClick={carregar}
                  className="inline-flex items-center gap-2 bg-slate-100 border border-slate-300 text-slate-700 hover:text-slate-900 rounded-lg px-3 py-2 text-sm"
                >
            ...
            <div className="rounded-xl overflow-hidden border border-slate-200">
              <table className="w-full text-left" style={{ minWidth: '620px' }}>
                <thead className="bg-slate-100 border-b border-slate-200 text-[11px] uppercase tracking-widest text-slate-500">
                  <tr>
                        ...
                        <tr key={colaborador.id} className="hover:bg-slate-50">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center border border-slate-300">
                                <UserRound className="w-4 h-4" />
                              </div>
                              <span className="text-slate-900 font-medium">{colaborador.nome}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="p-2 rounded-lg bg-slate-100 text-slate-600 hover:text-slate-900 border border-slate-300"
                                title="Editar"
                              >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-6">
          <div className="bg-white/90 border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <h2 className="text-slate-900 font-black text-lg tracking-tight">Leitor do Motorista</h2>
                <p className="text-sm text-slate-600">Use esta aba apenas para validar entradas por QR/NFC.</p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-50 border border-slate-300 rounded-xl px-3 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500"
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                className="px-3 py-2 rounded-lg bg-slate-100 border border-slate-300 text-slate-700 text-sm font-bold"
              >
                {isScannerOpen ? 'Parar Câmara' : 'Ler QR pela Câmara'}
              </button>
              <button
                type="button"
            ...
            {isScannerOpen && (
              <div className="mt-4 rounded-xl overflow-hidden border border-slate-300 bg-slate-100">
          ...
          <div className="bg-white/90 border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-slate-900 font-black text-lg tracking-tight">Pré-visualização da leitura</h3>
            </div>

            {checkinPreview ? (
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
                <p className="text-[11px] uppercase tracking-wider text-emerald-300 font-bold">Colaborador lido</p>
                <p className="text-slate-900 font-black text-xl mt-2">{checkinPreview.colaborador.nome}</p>
                <div className="mt-3 space-y-1 text-sm text-slate-700">
                  <p>Nº {checkinPreview.colaborador.numero}</p>
                  <p>Paragem: {checkinPreview.colaborador.paragem || 'Sem paragem'}</p>
                  <p>Método: {checkinPreview.request.metodo.toUpperCase()}</p>
                </div>
                <p className="text-slate-500 text-xs mt-3">
                  Expira: {new Date(checkinPreview.request.expires_at).toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
            ) : (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
                <p className="text-slate-700 font-bold">Nenhum QR lido ainda</p>
                <p className="text-sm text-slate-500 mt-2">
                  Abra a câmara ou introduza o token manualmente para validar o embarque do colaborador.
                </p>
              </div>
            )}
          </div>
