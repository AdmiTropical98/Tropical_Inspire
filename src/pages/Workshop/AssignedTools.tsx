...
export default function AssignedTools() {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {/* Search Bar */}
            <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 p-4 rounded-2xl">
                <div className="relative group max-w-2xl">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 group-focus-within:text-blue-500 transition-colors" />
                    <input
                        type="text"
                        placeholder="Procurar por ferramenta ou técnico..."
                        className="w-full bg-slate-950/50 border border-slate-800 rounded-xl pl-12 pr-4 py-3 text-slate-200 outline-none focus:ring-2 focus:ring-blue-600/40 transition-all"
                        value={searchTerm}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAssets.map(asset => {
                    ...
                    return (
                        <div key={asset.id} className="group bg-slate-900/30 hover:bg-slate-900/50 border border-slate-800/50 hover:border-blue-600/30 rounded-3xl overflow-hidden transition-all duration-500 flex flex-col">
                            <div className="p-6">
                                <div className="flex items-start justify-between mb-6">
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-800/50 px-2 py-0.5 rounded">
                                ...
                                <div className="space-y-4">
                                    {/* Technician Info */}
                                    <div className="flex items-center gap-4 bg-slate-950/60 p-4 rounded-2xl border border-slate-800/40 relative group/tech">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm font-bold text-white truncate">{technician?.nome || 'Desconhecido'}</p>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-slate-800/30 p-3 rounded-2xl border border-slate-800/50">
                            ...
                            {/* Action Bar */}
                            <div className="mt-auto p-4 bg-slate-950/20 border-t border-slate-800/50 flex items-center gap-2">
                                <button
                                    onClick={() => assignWorkshopAsset(asset.id, null)}
                                    className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-700 transition-all flex items-center justify-center gap-2"
                                >
