...
export default function Geofences() {
    ...
    const fetchData = async (isRefresh = false) => {
        ...
        try {
            ...
            setLoading(false);
            setRefreshing(false);
            setDebugInfo(debugLastResponse); // Sync debug info after fetch
        }
    }
};
    ...
    <div className="p-4 md:p-8 w-full mx-auto space-y-6 min-h-screen bg-[#0a0a0f]">
        ...
        <div className="mx-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl">
            ...
            <pre className="text-[10px] text-blue-400 font-mono overflow-x-auto whitespace-pre-wrap">
                {JSON.stringify(import('../../services/cartrack').then(m => m.debugLastResponse).catch(() => 'Loading...'), null, 2)}
                {/* Accessing variable directly via window for easier debug or just static text if module ensures sync */}
                {/* Better approach: */}
                Check Console or see below if captured:
            </pre>
        </div>
